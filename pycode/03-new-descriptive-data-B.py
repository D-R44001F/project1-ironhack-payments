...
groupby_df = (
    consolidated_df.groupby(['category', 'cash_request_id','type'], as_index=False)
    .agg({
        'cohort_month': 'first',  # Select first cohort_month
        'cash_request_id': 'first',  # Select first cash_request_id        
        'type': 'first',  # Select first cash_request_id        
        'status': 'count',  # Select first status
...
groupby_file_path = f'{PATH}groupby_ironhack_data_2.csv'
